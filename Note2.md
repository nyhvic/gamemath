...
카메라 뒤에 있는 점이 투영되어 이상하게 렌더링되는 현상  
triangle clipping - 뷰 공간 깊이(클립좌표 w값)이용해 삼각형 영역 자르기, 정면-0보다 큼, 뒷면-0보다 작음, 클립 좌표에서 진행  
잘라낼 영역(절두체 평면 밖)에 속한 점 구하기 - 점이 없으면 그대로 그림 - 점 1개 평면과 교차점 구해 두 삼각형으로 분할(정점 순서 지키기) - 2개 교차점 구해 갱신 - 3개 스킵  
절두체 평면마다 평면 외부 판별식(w와 값 비교), affine combination 이용해 절두체 평면(w=0,+-y,+-x,+-z)마다 아핀결합 계수 계산해 사용  
...
## 13-6
삼각형마다 절두체 평면에 대해 클리핑 수행  
평면마다 판별 식, 아핀결합 계수 식 사용  
